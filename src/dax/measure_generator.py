...
def generate_dax_for_mapped_visuals(
    ...
    for v in mapped_visuals:
        if v["html_type"] in ["kpi_card", "metric_card"]:
            ...
            if matched_field and matched_field in existing_col_names:
                ...
                formula = generate_dax_formula(title, table_name, None, "DIVIDE", num, den)
                measures.append({
                    "measure_name": title,
                    "table_name": table_name,
                    "column_name": num,
    ...
    log_event("dax", f"Generated {len(measures)} DAX measures for Power BI semantic model.")
    return measures
